...
type GithubConnectionRepoProxy = {
    info: (repoName: string) => Promise<Github.ReposGetResponse>;
    stars: (repoName: string) => Promise<Array<Github.ActivityListStargazersForRepoResponseItem>>;
    forks: (repoName: string) => Promise<Array<Github.ReposListForksResponseItem>>;
    pullRequests: (repoName: string) => Promise<Array<Github.PullsListResponseItem>>;
    pullRequestsComments: (repoName: string) => Promise<Array<Github.PullsListCommentsForRepoResponseItem>>;
    issues: (repoName: string) => Promise<Array<Github.IssuesListForRepoResponseItem>>;
    issueComments: (repoName: string) => Promise<Array<Github.IssuesListCommentsResponseItem>>;
    commits: (repoName: string) => Promise<Array<Github.ReposListCommitsResponseItem>>;
    contributors: (repoName: string) => Promise<any>;
    watches: (repoName: string) => Promise<Array<Github.ActivityListWatchersForRepoResponseItem>>;
...
type GithubConnectionUserProxy = {
    info: (username: string) => Promise<Github.UsersGetByUsernameResponse>;
}
...
export class GithubConnectionPool {
    ...
    private pool: Array<GithubConnection>;
    ...
    private genConnection(token: string, timeout: number): GithubConnection {
        ...
        connection.client.hook.before("request", options => {
            options.headers["User-Agent"] = "Github data provider for alibaba";
    ...
    private parseRepo(repoName: string): RepoName {
        ...
        return {
            owner,
            repo,
            error: (!owner || !repo) ? `Parse error for ${repoName}` : null
        };
    }

    private pagingWrapper<T>(repoName: string, func: (conn: Github, param: any) => Promise<Github.Response<Array<T>>>): Promise<Array<T>> {
    ...
    async getConnection(): Promise<Github> {
        let instance = this;
    ...
    public repo: GithubConnectionRepoProxy = {
        ...
        info: (repoName: string): Promise<Github.ReposGetResponse> => {
            return new Promise<Github.ReposGetResponse>(resolve => {
                let repoNameRet = this.parseRepo(repoName);
                let info: Github.ReposGetResponse = null;
                if (repoNameRet.error) {
                    this.logger(repoNameRet.error);
                    resolve(info);
                    return;
        ...
        stars: (repoName: string): Promise<Array<Github.ActivityListStargazersForRepoResponseItem>> => {
            return this.pagingWrapper(repoName, (conn: Github, param: any) => {
    ...
    public user: GithubConnectionUserProxy = {
        info: (username: string): Promise<Github.UsersGetByUsernameResponse> => {
            return new Promise<Github.UsersGetByUsernameResponse>(resolve => {
